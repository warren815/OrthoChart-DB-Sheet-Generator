...
function parseToothTag(inner) {
  var rawParts = inner.split('|').map(function (s) { return s.trim().replace(/v/gi, '^'); });
...
■ 작업 지침
[1] 블록 분리
- 텍스트에는 EMR 원본의 섹션 헤더(■ 주관적 소견 / ■ 객관적 소견 / ■ 치료 및 경과 / ■ 진단명 / ■ 진료계획 / ■ 기타 등)가 그대로 남아있습니다.
- ★블록 경계 판단 기준(가장 신뢰할 수 있는 기준): "■ 진단명"(그리고 그 아래 "Malocclusion, unspecified <...>" 문구)은 예외 없이 모든 블록에 정확히 한 번씩 등장합니다. "■ 기타"나 "* 예진자 : ..." 줄은 없는 블록도 있으니 블록의 끝을 판단하는 기준으로 삼지 마세요.
- 한 블록 안에서의 순서는 다음과 같습니다: (■ 주관적 소견/■ 객관적 소견/■ 치료 및 경과 등 실제 진료 내용) → ■ 진단명 → (있는 경우) ■ 진료계획 → (있는 경우) ■ 기타. 즉 "■ 진단명"은 그 블록 진료 내용의 뒤에 나오며, 진료계획과 기타는 진단명보다도 더 뒤(같은 블록 안)에 이어집니다.
- 블록을 나누는 실제 방법: "■ 진단명"이 나오면 그 블록의 핵심 내용은 끝난 것이지만, 바로 다음 블록으로 넘어가지 말고 이어지는 (있는 경우) 진료계획·기타까지 같은 블록으로 포함하세요. 그런 다음, 그 뒤에 다시 새로운 진료 내용 섹션(■ 주관적 소견/■ 객관적 소견/■ 치료 및 경과 등)이 나오는 지점부터를 다음 블록의 시작으로 보세요.
- ★주의(오분류 방지, 매우 중요): "■ 진료계획"과 그 아래 "n)"/"nn)" 줄들은 그 계획을 세운 "당일" 방문(직전의 ■ 진단명·진료 내용과 같은 블록)에 반드시 포함되어야 합니다. "다음 방문에 할 일"이라는 내용이라고 해서 다음 블록(다음 날짜)으로 옮기면 안 됩니다.
