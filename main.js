...
// Generate orthodontic sheet via Gemini
btnGenerate.addEventListener('click', async () => {
  ...
  try {
    setProgress(true, '차트 데이터를 분석하는 중입니다 (Gemini API 호출)...');
...
제공된 두 종류의 정보(1. EMR에서 복사한 진료기록 텍스트, 2. 차트 화면 캡처 이미지들)를 이용해, 손글씨로 작성하던 'Debonding Sheet(DB Sheet)' 스타일의 간결한 임상 타임라인 데이터를 JSON으로 추출해주세요.

■ 입력 데이터의 역할 — 두 입력의 용도가 서로 다릅니다. 반드시 구분해서 사용하세요.
1. 복사한 차트 텍스트 (내용의 유일한 출처):
   - EMR에서 날짜를 제외하고 본문만 복사한 텍스트로, 날짜 오름차순(시간 흐름 순)으로 정렬되어 있습니다.
   - S)/O)/Tx)/N)/NN)/C.C)/PI)/[Tx Plan] 등 진료 내용은 반드시 이 텍스트에서만 추출하세요.
2. 차트 화면 캡처 이미지 (오직 날짜 매핑 용도로만 사용):
   - 이미지는 각 텍스트 블록에 대응하는 "진료 날짜"를 알아내기 위한 참고 자료일 뿐입니다.
   - 이미지 안에 보이는 진료 내용(치식, 처치 내용 등)을 텍스트 대신 사용하거나, 텍스트에 없는 내용을 이미지에서 새로 추가하지 마세요.
   - 이미지와 텍스트의 내용이 서로 달라 보이더라도, content(S/O/Tx 등)는 항상 텍스트 쪽을 그대로 따르고 이미지에서는 날짜만 가져오세요.

■ 작업 순서: (1) 텍스트를 날짜 구분 없이 순서대로 각 진료 블록으로 나눈다 → (2) 각 블록의 내용을 아래 [2] 형식에 맞춰 정리한다 → (3) 이미지에서 날짜만 읽어 각 블록에 순서대로 매핑한다.

■ 작업 지침:
[1] 날짜 매핑 — ★매우 중요, 아래 규칙을 정확히 지키세요★
- 날짜는 오직 이미지에서만 가져오고, 이미지는 오직 날짜를 알아내는 용도로만 사용하세요(내용 추출에는 사용 금지).
- 이미지에는 두 종류의 날짜/시각 정보가 존재합니다. 반드시 (A)만 사용하고 (B)는 절대 사용하지 마세요.
  (A) 진료 날짜(사용할 것): 각 기록 블록 맨 위, 주황색/노란색 등으로 강조(하이라이트)된 헤더 바에 있는 날짜.
      형식 예: "2024.01.15  21세  교정과초진2" 또는 "2025.03.07  23세  교정과재진2"
      → 이 헤더 바의 "YYYY.MM.DD" 부분이 그 블록의 실제 내원(진료) 날짜입니다.
  (B) 서명/작성 시각(사용 금지): 각 기록 블록 하단이나 우측 구석에 작은 글씨로 표시된
      "이름 YYMMDD HH:MM / 이름 YYMMDD HH:MM" 형식의 타임스탬프(예: "정석기 240115 17:51 / 정석기 240115 17:51").
...
"""
    ...
    setProgress(false);
    alert('DB sheet 데이터 추출이 완료되었습니다! 프리뷰를 검토하고 워드 파일을 다운로드하세요.');
